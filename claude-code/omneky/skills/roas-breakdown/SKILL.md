...
| Question | Tool |
| --- | --- |
| Ranked breakdown / leaderboard (creative, campaign, channel, ad, …) | `get_performance_breakdown` (alias `get_dimension_summary`) |
| Day-by-day time series / charts | `get_daily_metrics` |
| What rose or fell vs the prior window | `get_performance_movers` (alias `get_trending`) |
...
| Confirm imported data | `check_reporting_data_available` (alias `data_available`) |
...
2. `list_brands` / `get_brand_summary` (alias `get_brand`) / `get_brand_details` — confirm which brand. Do not guess `brand_id`.
3. `check_reporting_data_available` (alias `data_available`) — confirm the brand has imported data (and which dates/channels) before querying.
...
- `get_performance_breakdown` (alias `get_dimension_summary`) needs `user_id`, `date_range` (`["YYYY-MM-DD", "YYYY-MM-DD"]`), `dimension`, `results_per_page`, `sort_metric`, `sort_order`. Pass `filters` as `[]` when unscoped — never null, and never a redundant `{dimension: "brand"}` filter.
- Common dimensions: `creative`, `campaign`, `channel`, `ad`, `ad_group`. Sort by `ROAS`, `CLICK_THROUGH_RATE`, or `SPEND` as asked.
- `get_performance_movers` (alias `get_trending`) needs `brand_id`, `user_id`, `dimension`, `metric`, `end_date`. Optional `period_days` (default 7).
...
- If `check_reporting_data_available` (alias `data_available`) says there is no import yet, say so instead of fabricating a table.
