...
| Full record | `get_product_details` (alias `fetch_product_details`) |
| Manual create | `create_product` |
| Update a completed product | `update_brand_product` (alias `upsert_brand_product`) |
...
- Full record: `get_product_details` (alias `fetch_product_details`) (`brand_id`, `product_id`).

## Manual create / update

- New product: `create_product` with `brand_id`, `product_name`, `product_description`. Optional: `price`, `benefits`, `offer`, `target_audience`, `product_url`, `product_type`, `industry_vertical`, `is_app`. Prefer this over `update_brand_product` (alias `upsert_brand_product`) for adds.
- Update a **completed** product: `update_brand_product` (alias `upsert_brand_product`) with `product_id`. Call `get_product_details` (alias `fetch_product_details`) first and pass current `product_name` / `product_description` if you are not changing them.
- Change only the product page URL: `update_product_url(brand_id, product_id, url)`. Confirm the URL first.

Do **not** use `update_brand_product` (alias `upsert_brand_product`) to finalize a URL scrape.
...
3. Confirm name, description, and at least one image (`request_user_decision` if needed; alias `ask_user`).
...
After catalogue work, route product creative to `creative-referral`, or launch with `launch-meta-ads` / `launch-google-tiktok-ads`.
